...
def update_parameters_with_adam(parameters, grads, v, s, t, learning_rate = 0.01,
    ...
    for l in range(1, L + 1):
        ...
        # parameters["b" + str(l)] = ...
        # YOUR CODE STARTS HERE
        parameters["W" + str(l)] = parameters["W" + str(l)] - (learning_rate * v_corrected["dW"+str(l)])/(epsilon + (s_corrected["dW"+str(l)])**0.5)
        parameters["b" + str(l)] = parameters["b" + str(l)] - (learning_rate * v_corrected["db"+str(l)])/(epsilon + (s_corrected["db"+str(l)])**0.5)
...
def model(X, Y, layers_dims, optimizer, learning_rate = 0.0007, mini_batch_size = 64, beta = 0.9,
          beta1 = 0.9, beta2 = 0.999,  epsilon = 1e-8, num_epochs = 5000, print_cost = True, decay=None, decay_rate=0.2):
    """
    3-layer neural network model which can be run in different optimizer modes.
...
# train 3-layer model
layers_dims = [train_X.shape[0], 5, 2, 1]
parameters = model(train_X, train_Y, layers_dims, optimizer = "adam", learning_rate = 0.1, num_epochs=5000, decay=update_lr)
...
# train 3-layer model
layers_dims = [train_X.shape[0], 5, 2, 1]
parameters = model(train_X, train_Y, layers_dims, optimizer = "momentum", learning_rate = 0.1, num_epochs=1001, decay=schedule_lr_decay)
